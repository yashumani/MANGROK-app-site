/**
 * Curated visual library for Mangrok recipe entry.
 * Emoji provide consistent, offline illustrations; custom text remains available.
 */
function expand(groups) {
  return Object.entries(groups).flatMap(([category, rows]) => rows.trim().split("\n").filter(Boolean).map(row => {
    const separator = row.indexOf("|");
    return Object.freeze({ icon: row.slice(0, separator), name: row.slice(separator + 1), category });
  }));
}
...
☕|Coffee grinder
🧇|Waffle maker`,
  "Serving & storage": `🍽️|Dinner plate
🥣|Serving bowl
🫖|Pitcher
🫙|Glass jar
🧊|Food container
