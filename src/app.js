...
function recipeCard(recipe) {
  ...
  return `<article class="recipe-card" data-id="${escapeHtml(recipe.id)}" data-privacy="${escapeHtml(recipe.privacy)}">
    ...
    <button class="favorite-button ${recipe.favorite ? "active" : ""}" data-action="favorite" aria-label="${recipe.favorite ? "Remove from" : "Add to"} favorites">★</button></div>
    ...
    <div class="card-tags">${recipe.tags.slice(0,4).map(tag => `<span class="tag">${escapeHtml(tag)}</span>`).join("")}${recipe.secret ? `<span class="sealed-badge">◆ Sealed</span>` : ""}</div>
    <div class="card-meta"><span>☷ ${recipe.ingredients.length} ingredients</span>${minutes ? `<span>◷ ${minutes} min</span>` : ""}<span>↻ r${recipe.revision}</span></div>
...
async function openShareCenter(recipe) {
  ...
  dialog.innerHTML = `<div class="modal-shell"><header><div><p class="eyebrow">Controlled access</p><h2>Share ${escapeHtml(recipe.title)}</h2></div><button class="icon-button" data-dismiss aria-label="Close">×</button></header>
...
async function bookOptions(requireApproval=false){const form=$("#book-form"),data=new FormData(form),recipes=selectedRecipes(form,"#print-recipe-list"),includeSecrets=Boolean(data.get("includeSecrets"));if(!recipes.length)throw new Error("Select at least one recipe.");if(includeSecrets){if(requireApproval&&!data.get("secretApproval"))throw new Error("Approve irreversible secret printing first.");await ensureSecrets(recipes);}
  return{title:String(data.get("title")||"Our Recipes"),dedication:String(data.get("dedication")||""),theme:String(data.get("theme")||"heritage"),recipes,includeSecrets,secretApprovalAt:includeSecrets&&data.get("secretApproval")?isoNow():null,unlockedSecrets:Object.fromEntries(state.unlockedSecrets)};}
...
function showPublicShare(recipeData,secret){const recipe=normalizeRecipe(recipeData);const dialog=document.createElement("dialog");dialog.className="modal wide-modal";dialog.innerHTML=`<div class="modal-shell"><header><div><p class="eyebrow">Shared with you</p><h2>${escapeHtml(recipe.title)}</h2></div><button class="icon-button" data-dismiss>×</button></header><div class="viewer-layout"><div><h3>Ingredients</h3><ul>${recipe.ingredients.map(v=>`<li>${escapeHtml(v)}</li>`).join("")}</ul></div><div><h3>Method</h3><ol>${recipe.steps.map(v=>`<li>${escapeHtml(v)}</li>`).join("")}</ol>${secret?`<div class="viewer-secret"><h3>Sealed note — shared intentionally</h3><p>${escapeHtml(secret)}</p></div>`:""}</div></div><p class="warning-note">This access may expire or be revoked. Visible information can still be copied outside Mangrok.</p></div>`;document.body.append(dialog);dialog.showModal();dialog.querySelector("[data-dismiss]").onclick=()=>dialog.close();dialog.addEventListener("close",()=>dialog.remove());}
